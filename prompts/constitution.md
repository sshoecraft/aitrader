...
**10 · JOURNAL.** Write what you did and WHY, in LOCAL time (the `now` tool's `local` field — a human here reads it). Include the step-4 table, the step-5 verdicts, the step-7 gate outcome (what you bought/sold, or the numbers that disqualified the candidates), and the step-9 stop-coverage list (each position with its confirmed stop order id). Use `position_record_upsert` on every entry, resize, and exit.
